    ...
    void FixedUpdate()
    {
        ...
        { 
            ...
            {   
                ...
                {
                    WeaponStats.rocketCounter++;
                }           
            }

            //SUMARY: Nếu bắn trúng BONUS với machine gun:
            if (hit.collider.CompareTag("BonusFireRate") && gameObject.CompareTag("PlayerBullet"))
            {
                hit.collider.GetComponent<EnemyHealth>().TakeDamage(WeaponStats.machineGun_1_Damage);
                ...
                //Particle when hit:
                GameObject hitEffect = EffectPool.instance.GetEffectPool(EnumforPooling.ObjectList.BonusHit);
                if (hitEffect != null)
                {
                    hitEffect.transform.position = hit.point;
            ...
            else if((hit.collider.CompareTag("Enemy") || hit.collider.CompareTag("HeavyEnemy")) && gameObject.CompareTag("PlayerRocket"))
            {
                ...
                if (rocketHitEffect != null)
                {
                    ...
                    rocketHitEffect.transform.localScale = Vector3.one * 5;
